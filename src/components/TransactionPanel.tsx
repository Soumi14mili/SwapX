...
export const TransactionPanel: React.FC<TransactionPanelProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {txStatus !== 'idle' && txStatus !== 'success' && txStatus !== 'failed' && (
        <motion.div
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-white/15">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
          <div>
            ...
            <p className="text-xs text-slate-400">On-chain Testnet swaps &amp; XLM payments</p>
          </div>
        ...
        {transactions.length === 0 ? (
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Execute a token swap or send XLM to generate Stellar Testnet transactions.
            </p>
          </div>
        ) : (
            ...
            {transactions.map((tx) => {
              ...
              return (
                ...
                >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded font-bold uppercase ${
                            ...
                            {isPayment
                              ? <><SendHorizonal className="w-2.5 h-2.5" /> Payment</>
                              : <><Zap className="w-2.5 h-2.5" /> Swap</>
                            }
                          </span>

                          <span className="font-bold text-white text-sm">
                            {isPayment
                              ? `${tx.fromAmount} XLM → ${tx.recipientAddress
                        ...
                        {isSuccess && (
                          <div className="text-xs text-slate-400 font-mono mt-0.5">
                            Ledger Block #{tx.ledgerBlock.toLocaleString()} · Fee: {tx.feePaidXlm}
                          </div>
                        )}

                        {/* Failed error message */}
                        {isFailed && tx.errorMessage && (
                          <div className="flex items-center gap-1.5 text-xs text-rose-400 mt-0.5 font-mono">
                            <AlertTriangle className="w-3 h-3 flex-shrink-0" />
                            <span className="truncate max-w-[260px]">{tx.errorMessage}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Hash row — only for successful txs with a real hash */}
                  {isSuccess && tx.hash && (
